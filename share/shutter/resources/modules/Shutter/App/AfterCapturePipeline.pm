...
sub build_config_widget ($self) {
    ...
    my $desc = Gtk3::Label->new(
        "Define a sequence of actions to execute automatically after each screenshot."
    );
    ...
    $toggle_renderer->signal_connect('toggled' => sub {
        my ($cell, $path_str) = @_;
    ...
    $extra_renderer->signal_connect('edited' => sub {
    ...
    $down_btn->signal_connect('clicked' => sub {
        my $sel  = $tv->get_selection;
        my (undef, $iter) = $sel->get_selected;
        return unless $iter;
