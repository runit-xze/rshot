package Shutter::App::Handlers::Screenshot_VideoRecord;
...
sub evt_video_record ($self, $widget, $data, $folder_from_config, $extra) {
    my $cli = $self->cli;
    my $sc = $cli->sc;
    ...
    if (!$x11_supported) {
        ...
        $sd->dlg_error_message($d->get("Can't take video screenshots without X11 server"), $d->get("Failed"));
        return TRUE;
    }
    ...
    Glib::Timeout->add($cli->{_hide_time}->get_value || 250, sub {
...
sub _start_capture_flow ($self, $data, $folder_from_config, $extra) {
    ...
    } elsif ($data eq 'video_window' || $data eq 'tray_video_window') {
        # Select window
        my $screenshooter = Shutter::Screenshot::Window->new(
            $sc, FALSE, 0, FALSE,
            FALSE, FALSE, 800, 600,
            0, "window", FALSE, FALSE,
            FALSE, FALSE
        );
...
sub _show_countdown ($self, $region, $on_done) {
    ...
    my $countdown = $sm->get_setting('video', 'countdown') // 3;
    ...
    $overlay->signal_connect('draw' => sub {
        my ($widget, $cr) = @_;
...
sub _begin_recording ($self, $data, $region, $folder_from_config) {
    ...
    my $fps = $sm->get_setting('video', 'fps') // 30;
    my $max_duration = $sm->get_setting('video', 'max_duration') // 30;
    ...
    if ($sc->get_export_filename) {
        ...
        my $giofile = $cli->handlers->get('Util_Get')->fct_get_next_filename($filename_value, $folder, "mp4");
        $output_path = $shf->utf8_decode(uri_unescape($giofile->get_path));
    }
    
    print STDERR "[DEBUG] Output path generated: " . ($output_path // 'undef') . "\n";
    
    my $recorder = Shutter::Screenshot::VideoRecorder->new(
        cli => $cli,
        _common => $sc,
        region => $region,
        fps => $fps,
...
sub _on_recording_done ($self, $video_path) {
    ...
    my $giofile = Glib::IO::File::new_for_path($video_path);
