...
sub _create_factory {
	...
	$factory->add('shutter-ellipse', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-ellipse.png')));

	$factory->add('shutter-eraser', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-eraser.png')));

	$factory->add('shutter-freehand', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-freehand.png')));

	$factory->add('shutter-highlighter', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-highlighter.png')));

	$factory->add('shutter-pointer', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-pointer.png')));

	$factory->add('shutter-rectangle', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-rectangle.png')));

	$factory->add('shutter-line', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-line.png')));

	$factory->add('shutter-arrow', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-arrow.png')));

	$factory->add('shutter-text', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-text.png')));

	$factory->add('shutter-censor',   Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-censor.png')));
	$factory->add('shutter-pixelize', Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-pixelize.png')));
	$factory->add('shutter-number',   Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/draw-number.png')));
	$factory->add('shutter-crop',     Gtk3::IconSet->new_from_pixbuf(Gtk3::Gdk::Pixbuf->new_from_file($self->dicons . '/transform-crop.png')));
...
sub _create_main_actions {
	...
	my @main_actions = (
		...
		["Delete", 'gtk-delete', undef, "Delete", $self->gettext->get("Delete current object"), sub { $self->app->{_state_manager}->clear_item_from_canvas($self->app->current_item) }],
		[
			"Clear",
			'gtk-clear',
			undef,
		...
		["Stop",  'gtk-stop',  undef, "Escape",     $self->gettext->get("Abort current mode"), sub { $self->app->{_state_manager}->abort_current_mode }],
		["Close", 'gtk-close', undef, "<control>Q", $self->gettext->get("Close this window"),  sub { $self->app->{_state_manager}->quit(TRUE) }],
		[
			"Save",
			'gtk-save',
			undef,
...
sub _create_drawing_actions {
	my $self            = shift;
	my @drawing_actions = (
		["Select",      'shutter-pointer',     $self->gettext->get("Select"),      "<alt>0",       $self->gettext->get("Select item to move or resize it"),                                10],
		["Freehand",    'shutter-freehand',    $self->gettext->get("Freehand"),    "<alt>1",       $self->gettext->get("Draw a freehand line"),                                            20],
		["Highlighter", 'shutter-highlighter', $self->gettext->get("Highlighter"), "<alt>2",       $self->gettext->get("Highlighter"),                                                     30],
		["Line",        'shutter-line',        $self->gettext->get("Line"),        "<alt>3",       $self->gettext->get("Draw a straight line"),                                            40],
		["Arrow",       'shutter-arrow',       $self->gettext->get("Arrow"),       "<alt>4",       $self->gettext->get("Draw an arrow"),                                                   50],
		["Rect",        'shutter-rectangle',   $self->gettext->get("Rectangle"),   "<alt>5",       $self->gettext->get("Draw a rectangle"),                                                60],
		["Ellipse",     'shutter-ellipse',     $self->gettext->get("Ellipse"),     "<alt>6",       $self->gettext->get("Draw a ellipse"),                                                  70],
		["Text",        'shutter-text',        $self->gettext->get("Text"),        "<alt>7",       $self->gettext->get("Add some text to the screenshot"),                                 80],
		["Censor",      'shutter-censor',      $self->gettext->get("Censor"),      "<alt>8",       $self->gettext->get("Censor portions of your screenshot to hide private data"),         90],
		["Pixelize",    'shutter-pixelize',    $self->gettext->get("Pixelize"),    "<alt><ctrl>8", $self->gettext->get("Pixelize selected areas of your screenshot to hide private data"), 100],
		["Number",      'shutter-number',      $self->gettext->get("Number"),      "<alt>9",       $self->gettext->get("Add an auto-increment shape to the screenshot"),                   110],
		["Crop",        'shutter-crop',        $self->gettext->get("Crop"),        "<alt>c",       $self->gettext->get("Crop your screenshot"),                                            120],
	);
