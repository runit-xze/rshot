		...
		if ($session_screens->{$key}->{'giofile'}->query_exists) {
			...
			if ($session_screens->{$key}->{'is_unsaved'}) {
				...
				$session_screens->{$key}->{'tab_label'}->set_text("[" . $session_screens->{$key}->{'tab_indx'} . "] - " . "*" . $session_screens->{$key}->{'name'})
					if $session_screens->{$key}->{'tab_label'};
			} else {
			...
			if (defined $xdo && $xdo eq 'block') {
				Shutter::App::Core::FileSystemAPI->new->remove($tmpfilename);
			} elsif (defined $xdo && $xdo eq 'clear') {
				while (defined $session_screens->{$key}->{'undo'} && scalar @{$session_screens->{$key}->{'undo'}} > 0) {
					Shutter::App::Core::FileSystemAPI->new->remove(shift @{$session_screens->{$key}->{'undo'}});
				}
				while (defined $session_screens->{$key}->{'redo'} && scalar @{$session_screens->{$key}->{'redo'}} > 0) {
					Shutter::App::Core::FileSystemAPI->new->remove(shift @{$session_screens->{$key}->{'redo'}});
				}
			...
			unless ($session_screens->{$key}->{'no_thumbnail'}) {
				my $max_size = 100;
				$thumb_view =
					  ($im_width <= $max_size && $im_height <= $max_size)
					? ($pixbuf // $lp_ne->load($session_screens->{$key}->{'giofile'}->get_path))
					: $lp_ne->load($session_screens->{$key}->{'giofile'}->get_path, $max_size, $max_size)
					if $lp_ne;
