...

{
	...
	package MockWindowAS;
	sub get_geometry { (0, 0, 500, 400, 24) }
}
...

{
	...
	use overload '>=' => sub {
		...
		return scalar grep { $_ eq $other } @{$self->{flags}};
	}, fallback => 1;
	sub new { my ($class, @flags) = @_; bless {flags => \@flags}, $class }
}
...
subtest 'returns early when mode is censor' => sub {
	...
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 10, y => 10);
...
subtest 'does not scroll when inside bounds' => sub {
	...
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new(), x => 10, y => 10);
...
subtest 'scrolls right when near right edge' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 450, y => 200);
...
subtest 'scrolls down when near bottom edge' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 200, y => 350);
...
subtest 'scrolls diagonally when near bottom-right corner' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 450, y => 350);
...
subtest 'scrolls left when near left edge' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 50, y => 200);
...
subtest 'scrolls up when near top edge' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 200, y => 50);
...
subtest 'scrolls diagonally when near top-left corner' => sub {
	...
	$dt->_scrolled_window(MockScrolledWindowAS->new);
	my $consumer = MockConsumerAS->new(drawing_tool => $dt);
	my $ev       = MockEventAS->new(state => MockStateAS->new('button1-mask'), x => 50, y => 50);
