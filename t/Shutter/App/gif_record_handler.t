## no critic (Subroutines::RequireFinalReturn Modules::ProhibitMultiplePackages Modules::RequireEndWithOne Modules::RequireExplicitPackage Modules::RequireFilenameMatchesPackage)
use strict;
use warnings;
use v5.40;
...
subtest "Construction with cli attribute" => sub {
	...
	my $handler  = Shutter::App::Handlers::Screenshot_GifRecord->new(cli => $mock_cli,);
...
subtest "Module is a Moo class" => sub {
	...
	my $handler = Shutter::App::Handlers::Screenshot_GifRecord->new(cli => bless({}, 'MockCLI'),);
...
subtest "evt_gif_record method exists" => sub {
	...
	ok(Shutter::App::Handlers::Screenshot_GifRecord->can('evt_gif_record'), "evt_gif_record method is defined");
};
...
subtest "x11_supported=false triggers error dialog and returns TRUE" => sub {
	...
	my $handler = Shutter::App::Handlers::Screenshot_GifRecord->new(cli => MockCLI2->new(),);
	...
	like($dialog_calls[0]->{msg}, qr/X11/i, "Error message mentions X11");
};
