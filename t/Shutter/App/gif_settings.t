...
subtest "Round-trip for all GIF setting keys" => sub {
    ...
    for my $key (sort keys %test_values) {
        ...
        is($gs->get_gif_setting($key), $test_values{$key},
           "Round-trip for '$key': set $test_values{$key}, got $test_values{$key}");
    }
};
...
subtest "get_gif_setting for unknown key returns undef" => sub {
    ...
    my $gs = Shutter::App::GlobalSettings->new();
    is($gs->get_gif_setting('nonexistent_key'), undef,
       "Unknown key 'nonexistent_key' returns undef");
    is($gs->get_gif_setting(''), undef,
       "Empty string key returns undef");
};
