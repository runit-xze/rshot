...
subtest "copy_image with undef pixbuf does not crash" => sub {
    ...
    is(scalar @{$clipboard->{calls}}, 0,
       "Clipboard set_image NOT called when pixbuf is undef");
};
...
subtest "copy_filename with defined filename works" => sub {
    ...
    is_deeply(
        $clipboard->{calls},
        [['set_text', '/tmp/screenshot.gif']],
        "Clipboard set_text called with correct filename"
    );
};
...
subtest "open_in_editor works regardless of pixbuf" => sub {
    ...
    is($editor_received, '/tmp/screenshot.gif',
       "editor_cb called with correct filename");
};
